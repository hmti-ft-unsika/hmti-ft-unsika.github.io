...
├── style.css            # tampilan (tema dark navy + gradient, terinspirasi PIKA HMTI)
...
- Kolom H21–H78 pada `template.xlsx` (skor tiap kegiatan) diisi otomatis dari
  ...
  format resmi tetap terjaga. Identitas mahasiswa ditulis ke `F12`/`F13`/`F16`
  dan total akhir ke `D82`, mengikuti tata letak template versi terbaru.
- Rumus **Jumlah Skor Akhir SKP** (sel `D82`) pada template asli menjumlahkan
...
- Kalimat "Pada hari ini, ..." (sel `B10`) dan baris tanda tangan "Karawang,
  ..." (sel `G86`) diisi otomatis dengan hari & tanggal saat file diunduh,
  memakai format terbilang Bahasa Indonesia (mis. "Senin, tanggal Sepuluh
  bulan Juli tahun Dua ribu dua puluh empat").
- Beberapa sel di template asli punya warna font tidak konsisten (sebagian
  sel kolom SKOR PENILAIAN berwarna merah, sel tanggal tanda tangan tidak
  punya font eksplisit). Skrip ini menormalkan semua sel yang diisi jadi satu
  font (Aptos Narrow, hitam) agar hasil akhirnya rapi dan seragam.
- Tampilan (tema navy gelap, gradient biru–ungu–pink, dan watermark teks
  tersamar) sengaja dibuat senada dengan situs PIKA HMTI
  (`hmti.ft.unsika.ac.id/pika`), memakai logo UNSIKA, HMTI, dan Kabinet
  Sahitya Ardaya yang ditaruh di `assets/img/`. Kalau logo diganti, cukup
  timpa file dengan nama yang sama atau ubah path-nya di `index.html`.
