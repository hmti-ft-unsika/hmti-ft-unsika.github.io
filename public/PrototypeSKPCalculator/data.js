...
// Mengubah angka menjadi kata (terbilang), dipakai untuk kalimat
// "Pada hari ini, <Hari>, tanggal <terbilang> bulan <Bulan> tahun <terbilang> ..."
// persis seperti format resmi pada Berita Acara Verifikasi SKP.
function angkaKeTerbilang(n) {
  ...
  if (n < 100) {
    const sisa = n % 10;
    return angkaKeTerbilang(Math.floor(n / 10)) + " puluh" + (sisa !== 0 ? " " + angkaKeTerbilang(sisa) : "");
  }
  if (n < 200) return "seratus" + (n - 100 !== 0 ? " " + angkaKeTerbilang(n - 100) : "");
  if (n < 1000) {
    const sisa = n % 100;
    return angkaKeTerbilang(Math.floor(n / 100)) + " ratus" + (sisa !== 0 ? " " + angkaKeTerbilang(sisa) : "");
  }
  if (n < 2000) return "seribu" + (n - 1000 !== 0 ? " " + angkaKeTerbilang(n - 1000) : "");
  const sisaRibu = n % 1000;
  return angkaKeTerbilang(Math.floor(n / 1000)) + " ribu" + (sisaRibu !== 0 ? " " + angkaKeTerbilang(sisaRibu) : "");
}
...
// Kalimat lengkap: "Senin, tanggal Sepuluh bulan Juli tahun Dua ribu dua puluh empat"
function kalimatHariTanggal(date) {
  const hari = NAMA_HARI[date.getDay()];
  const tanggalKata = kapital(angkaKeTerbilang(date.getDate()));
  const bulan = NAMA_BULAN[date.getMonth()];
  const tahunKata = kapital(angkaKeTerbilang(date.getFullYear()));
  return `${hari}, tanggal ${tanggalKata} bulan ${bulan} tahun ${tahunKata}`;
}

// Format singkat untuk baris tanda tangan: "27 Juli 2026"
function tanggalSingkat(date) {
